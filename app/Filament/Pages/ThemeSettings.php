                        ...
                        ->schema([
                            ...
                            Section::make('ثبت‌نام، فاکتور و پرداخت خودکار')
                                ->description(new HtmlString(
                                    '<p class="text-sm text-gray-600 dark:text-gray-400">اگر پس از تأیید پرداخت در VPNMarket فاکتور در XMPlus همچنان Pending ماند، یعنی Client API فاکتور را با درگاه انتخاب‌شده نبسته است. در بسیاری از پنل‌ها باید در لیست فاکتورهای XMPlus دستی «Confirm» بزنید مگر اینکه شناسهٔ درگاه «کسر از موجودی نماینده» یا معادل آن را داشته باشید و در فیلد زیر بگذارید.</p>'
                                    .'<p class="text-sm mt-3 text-gray-600 dark:text-gray-400"><strong>درگاه سفارشی روی سرور XMPlus (برای تست خودکار بدون ورست به پنل):</strong> طبق '
                                    .'<a href="https://docs.xmplus.dev/settings/gateways.html" target="_blank" rel="noopener" class="text-primary-600 underline">مستندات Gateways</a>'
                                    .' می‌توانید کلاس درگاه را در <code class="text-xs">/src/Services/Gateway/</code> اضافه کنید (متدهای <code class="text-xs">pay($order)</code> و <code class="text-xs">notify</code> و غیره). هدف: وقتی VPNMarket همان <code class="text-xs">invoice/pay</code> را با این درگاه می‌زند، داخل پنل همان منطقی اجرا شود که با تأیید دستی — یعنی فاکتور Paid شود و در <code class="text-xs">account/info</code> سرویس و sublink ساخته شود (برخلاف به‌روزرسانی خام MySQL روی جدول invoice). بعد از فعال‌سازی درگاه در ادمین XMPlus، شناسهٔ عددی آن را از <code class="text-xs">POST /api/client/gateways</code> بگیرید و در فیلد «شناسه درگاه برای پرداخت خودکار فاکتور» وارد کنید.</p>'
                                ))
                                ->schema([
                                    ...
                                    TextInput::make('xmplus_auto_pay_gateway_id')
                                        ...
                                        ->numeric()
                                        ->helperText('برای خریدهایی که پول در VPNMarket گرفته شده (کیف پول، Plisio، کارت/تتر بعد از تأیید ادمین): باید شناسهٔ عددی درگاهی باشد که با یک بار فراخوانی invoice/pay فاکتور را از مسیر واقعی پنل ببندد و سرویس بسازد — معمولاً «موجودی/اعتبار نماینده»، یا درگاه سفارشی که خودتان در XMPlus نوشته‌اید (ر.ک. توضیح بالا و https://docs.xmplus.dev/settings/gateways.html). Card2Card و PayPal مشتری معمولاً کافی نیستند. اگر فاکتور Pending ماند، در پنل Confirm بزنید و در VPNMarket دوباره «تایید و اجرا» کنید. نام روش پرداخت روی فاکتور XMPlus همان درگاهی است که این شناسه به آن اشاره می‌کند.'),
                                ]),
                            Section::make('همگام‌سازی جدول invoice در MySQL پنل XMPlus')
                                ->description(new HtmlString(
                                    '<p class="text-sm text-gray-600 dark:text-gray-400">Client API معمولاً دکمهٔ «Confirm» پنل را ندارد. اگر بعد از تأیید پرداخت در VPNMarket فاکتور در XMPlus با <code>status=0</code> می‌ماند، می‌توانید با یک اتصال MySQL <strong>محدود</strong> همان ردیف را به <code>status=1</code> بزنید (مثل نمونهٔ <code>invoice.sql</code>). <strong>توجه:</strong> در بسیاری از نصب‌ها فقط این کار فاکتور را در API Paid نشان می‌دهد اما <strong>سرویس و لینک نمی‌سازد</strong>؛ برای خودکارسازی کامل، درگاه موجودی نماینده یا <a href="https://docs.xmplus.dev/settings/gateways.html" target="_blank" rel="noopener" class="text-primary-600 underline">درگاه سفارشی</a> در XMPlus بهتر است.</p>'
