                    ...
                    Tabs\Tab::make('پنل XMPlus (Client API)')
                        ...
                        ->schema([
                            ...
                            Section::make('ثبت‌نام، فاکتور و پرداخت خودکار')
                                ->description('اگر پس از تأیید پرداخت در VPNMarket فاکتور در XMPlus همچنان Pending ماند، یعنی Client API فاکتور را با درگاه انتخاب‌شده نبسته است. در بسیاری از پنل‌ها باید در لیست فاکتورهای XMPlus دستی «Confirm» بزنید مگر اینکه از پشتیبانی XMPlus شناسهٔ درگاه «کسر از موجودی نماینده» را بگیرید و در فیلد زیر بگذارید.')
                                ->schema([
                                    ...
                                    TextInput::make('xmplus_auto_pay_gateway_id')
                                        ...
                                        ->numeric()
                                        ->helperText('برای خریدهایی که پول در VPNMarket گرفته شده (کیف پول، Plisio، کارت/تتر بعد از تأیید ادمین): باید شناسهٔ عددی درگاهی باشد که با یک بار فراخوانی invoice/pay فاکتور را Paid کند — معمولاً «موجودی/اعتبار نماینده» در خود XMPlus، نه Stripe/PayPal مشتری. اگر چنین درگاهی در API شما نیست یا موجودی صفر است، فاکتور Pending می‌ماند و باید مثل اسکرین‌شات در پنل XMPlus روی Confirm بزنید؛ سپس در صورت نیاز سفارش را در VPNMarket دوباره «تایید و اجرا» کنید تا لینک همگام شود. نکته: روی فاکتور XMPlus، نام روش پرداخت همان درگاهی است که این شناسه به آن اشاره می‌کند؛ اگر «PayPal» می‌بینید یعنی اینجا اشتباهاً شناسهٔ درگاه PayPal گذاشته‌اید، نه اینکه مشتری PayPal زده باشد.'),
                                ]),
